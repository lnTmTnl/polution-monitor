<template>
  <div class="data-table">
    <div class="table-top" :class="{ hidden: !isDisplayTableTop }">
      <div class="left">
        <input type="text" name="search-box" class="search-box" v-model="searchInput"/>
        ...
        <tbody>
          ...
          >
            <td
              ...
              :style="{color: index < tdColors.length && indextd < tdColors[index].length ? (tdColors[index][indextd]) : '204969'}"
...
export default {
  ...
  methods: {
    ...
    onSearchBtnClick () {
      ...
      keys.forEach(key => {
        searchedDatas = searchedDatas.concat(this.data.filter(item => (item[key] + '') === this.searchInput))
      })
